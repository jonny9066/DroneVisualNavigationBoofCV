 ...
 * @author Peter Abeles
 */
public class KltFrameByFrame<T extends ImageGray<T>, D extends ImageGray<D>> {
	// type of input image
	Class<T> imageType;
	Class<D> derivType;
	...
	public KltFrameByFrame(Class<T> imageType, int pause ) {
	...
	public static void main( String[] args ) throws FileNotFoundException {
		...
		SimpleImageSequence<GrayF32> sequence =
				media.openVideo("resources/example/zoom.mjpeg", ImageType.single(imageType));
		pause = 100;
//				media.openCamera(null,640,480,ImageType.single(imageType)); pause = 5;
		sequence.setLoop(true);

		var app = new KltFrameByFrame<>(imageType, pause);
